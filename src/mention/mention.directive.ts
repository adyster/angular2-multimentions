 ...
 *
 * Modifications: Added support for multiple triggerChars & item lists
 */
@Directive({
  selector: '[mention]',
  host: {
...
export class MentionDirective {
  ...
  keyHandler(event: any, nativeElement: HTMLInputElement = this._element.nativeElement) {
    ...
    if (this.triggerChars.indexOf(charPressed) > -1) {
      this.startPos = pos;
      this.listIndex = this.triggerChars.indexOf(charPressed);
      ...
      if (!event.shiftKey &&
        ...
        if (event.keyCode === KEY_SPACE) {
          ...
          if (event.keyCode === KEY_TAB || event.keyCode === KEY_ENTER) {
            ...
            insertValue(nativeElement, this.startPos, pos,
              this.mentionSelect(this.triggerChars[this.listIndex] + this.searchList.activeItem), this.iframe);
        ...
        if (event.keyCode === KEY_LEFT || event.keyCode === KEY_RIGHT) {
          ...
          let matches = this.items[this.listIndex].filter(e => e.match(regEx) != null);
  ...
  showSearchList(nativeElement: HTMLInputElement) {
    if (this.searchList == null) {
      ...
      this.searchList.items = this.items[this.listIndex];
